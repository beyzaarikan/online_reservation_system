...
public class LoginPage extends BasePanel {
    ...
    public void setupUI() {
        ...
        backButton.addActionListener(e -> {
            dispose();
            // WelcomePage'i import etmeniz gerekecek
            try {
                ...
                System.out.println("WelcomePage not found, exiting...");
                System.exit(0);
            }
        });
    ...
    private void login() {
        ...
        if (isAdminLogin) {
            ...
            // Regular user login (simulate authentication)
            if (password.length() >= 6) { // Basic validation
                PageComponents.showStyledMessage("Welcome!", "Login successful!", this);
                dispose();
                new MainMenuPage(false).display(); // false for regular user
            } else {
                PageComponents.showStyledMessage("Error", "Invalid credentials!", this);
                return;
            }
